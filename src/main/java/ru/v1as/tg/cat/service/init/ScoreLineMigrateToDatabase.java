...
public class ScoreLineMigrateToDatabase {
    ...
    public void init2() {
        ...
        for (ScoreLine line : lines) {
            ...
            if (owner == null || chat == null) {
                continue;
            }
            CatUserEvent event =
                    new CatUserEvent(
                            chat, owner, line.getId(), line.getIsReal() ? REAL : CURIOS_CAT, cats);
            events.add(event);
            if (cats.getAmount() != 0) {
                final BigDecimal catsRewards =
                        CAT_REWARD.multiply(new BigDecimal(cats.getAmount()));
                final ResourceEvent resourceEvent =
                        new ResourceEvent(MONEY, catsRewards, event, owner, chat);
                resourceEvent.setDate(line.getDate());
